  ...
  const offices = ["Office A", "Office B", "Office C"]; // Add your office names here
  const departments = ["Department X", "Department Y", "Department Z"]; // Add your department names here
  ...
  const ManageItem = () => {
    ...
    const [selectedOffice, setSelectedOffice] = useState(""); // New state for Office
    const [selectedDepartment, setSelectedDepartment] = useState(""); // New state for Department
      ...
      if (editItem && editValue && editQuantity > 0 && editAmount >= 0 && editRequestedDate && editSupplier && editItemType) {
        try {
          ...
          await updateDoc(itemDoc, {
            text: editValue,
            college: editCollege || null, // Optional: Set to null if not selected
      ...
      <div className="manage-item-container">
                  ...
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.id}>
                        <td>{item.text}</td>
                        <td>{item.college}</td>
                        <td>{item.office || "N/A"}</td> {/* Display N/A if not provided */}
                        <td>{item.department || "N/A"}</td> {/* Display N/A if not provided */}
                        <td>{item.quantity}</td>
                        <td>{item.amount}</td>
        ...
        {editItem && (
          <div className="edit-item">
            ...
            <select value={editOffice} onChange={(e) => setEditOffice(e.target.value)}>
              <option value="">Select Office (Optional)</option>
              {offices.map((office) => (
                <option key={office} value={office}>{office}</option>
              ))}
